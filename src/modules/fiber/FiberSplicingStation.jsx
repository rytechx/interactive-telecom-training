...
import { FusionSplicer } from './FiberTools.jsx'
import {
...
function PreparedFiberEnd({ fiberRef, side, label, loaded, fused }) {
  return (
    <group ref={fiberRef}>
      <FiberSegment
        startX={side * 1.18}
...
function SplicerDisplay({
  ...
  if (currentStep === FIBER_PROCEDURE_STEPS.ALIGNING) {
    title = 'ALIGNING'
  ...
  return (
    <Html position={[0, 1.61, -0.44]} center>
...
export default function FiberSplicingStation({
  ...
  useEffect(() => {
    ...
    if (fiberPairRef.current) {
      fiberPairRef.current.position.set(
        ...(fusedFiberRemoved ? REMOVED_PAIR_POSITION : [0, 0, 0]),
      )
    }
  ...
  useFrame((_, delta) => {
    ...
    const duration =
      animationType === ANIMATION_TYPES.LOAD_A ||
      animationType === ANIMATION_TYPES.LOAD_B
        ? FIBER_LOADING_DURATION
        : animationType === ANIMATION_TYPES.CLAMP ||
          ...
          : animationType === ANIMATION_TYPES.CLOSE_LID ||
            ...
            ? FIBER_LID_DURATION
            : animationType === ANIMATION_TYPES.ALIGN
              ? FIBER_ALIGNMENT_DURATION
              : animationType === ANIMATION_TYPES.FUSE
                ? FIBER_FUSION_DURATION
                : FIBER_REMOVAL_DURATION
